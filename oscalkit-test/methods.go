...
func DownloadCatalog(url string) error {
	save := strings.Split(url, "/")
	err := os.Mkdir("./tmp", 0777)
	if err != nil {
		return err
	}
	filename := "./tmp/" + save[len(save)-1]
	...
	defer catalog.Close()
	finalurl := "https://raw.githubusercontent.com/usnistgov/OSCAL/master/content/nist.gov/SP800-53/rev4/" + save[len(save)-1]
	println("Downloading catalog from URL: " + finalurl)
	data, err := http.Get(finalurl)
	if err != nil {
		return err
	}
...
// ProfileParcing method to parse the profile and return the controls and subcontrols
func ProfileParcing(parsedProfile *profile.Profile) []string {
...
func ProfileProcessing(parsedProfile *profile.Profile) map[string][]string {
	...
	for l := 0; l < len(parsedProfile.Imports); l++ {
		...
		DownloadCatalog(parsedProfile.Imports[l].Href.Path)
		...
		if check == "Catalog" {
			...
			ProfileControls := ImportParcing(parsedProfile, parsedProfile.Imports[l].Href.Path)

			catalog := "./tmp/" + save[len(save)-1]
			...
			save1 := ImportParcing(parsedProfile, parsedProfile.Imports[l].Href.Path)
...
// ImportParcing method to parse the profile and return the controls and subcontrols
func ImportParcing(parsedProfile *profile.Profile, link string) []string {
